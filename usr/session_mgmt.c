...
__iscsi_login_portal(void *data, struct list_head *list, struct node_rec *rec)
{
	...
	sess_log_connect(1, NULL, "Logging in to [iface: %s, target: %s, portal: %s,%d]%s",
	...
	if (list) {
		...
		if (!async_req)
			sess_log_connect(1, NULL, "Could not allocate memory for async login "
...
int iscsi_login_portal(void *data, struct list_head *list, struct node_rec *rec)
{
	...
	for (i = session_count; i < rec->session.nr_sessions; ++i) {
		sess_log_connect(1, NULL, "%s: Creating session %d/%d", rec->iface.name,
