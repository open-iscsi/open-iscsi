...
/* TODO(Gris Ge): Add more info and debug message */
...
struct iscsi_session {
	...
	char targetname[_ISCSI_NAME_MAX_LEN + 1];
...
_iscsi_getter_func_gen(iscsi_session, targetname, const char *);
		      ...
		      struct iscsi_session **se)
{
	...
	char sys_con_dir_path[PATH_MAX];
	uint32_t host_id = 0;
	...
	snprintf(sys_con_dir_path, PATH_MAX, "%s/connection%" PRIu32 ":0",
		 _ISCSI_SYS_CONNECTION_DIR, sid);
	/* ^ BUG(Gris Ge): ':0' here in kernel is referred as connection id.
	 *		   but the openiscsi assuming it's always 0, need
	 *		   investigation.
	 */
	...
	*se = (struct iscsi_session *)
	...
	(*se)->sid = sid;
	/* TODO(Gris Ge): Should follow iscsi_sysfs_get_sessioninfo_by_id()
	 *		  on error handling
	 */
	_good(_sys_prop_get_str(ctx, sys_se_dir_path, "targetname",
				(*se)->targetname,
				sizeof((*se)->targetname)/sizeof(char)),
	      rc, out);

	_good(_sys_prop_get_str(ctx, sys_se_dir_path,
				      "username",
				      (*se)->username,
				      sizeof((*se)->username)/
				      sizeof(char)), rc, out);
	...
	_good(_sys_prop_get_str(ctx, sys_se_dir_path,
				      "username_in",
				      (*se)->username_in,
				      sizeof((*se)->username_in)/
				      sizeof(char)), rc, out);

	_good(_sys_prop_get_str(ctx, sys_se_dir_path,
				      "password_in",
				      (*se)->password_in,
				      sizeof((*se)->password_in)/
				      sizeof(char)), rc, out);

	_good(_sys_prop_get_i32(ctx, sys_se_dir_path,
				      "recovery_tmo",
				      &((*se)->recovery_tmo)), rc, out);

	if (_sys_prop_get_i32(NULL /* Ignore error */, sys_se_dir_path,
			      "lu_reset_tmo", &((*se)->lu_reset_tmo))
	    != LIBISCSI_OK)
		(*se)->lu_reset_tmo = -1;

	if (_sys_prop_get_i32(NULL /* Ignore error */, sys_se_dir_path,
			      "tgt_reset_tmo", &((*se)->tgt_reset_tmo))
	    != LIBISCSI_OK)
		(*se)->tgt_reset_tmo = -1;

	if (_sys_prop_get_i32(NULL /* Ignore error */, sys_se_dir_path,
			      "abort_tmo", &((*se)->abort_tmo))
	    != LIBISCSI_OK)
		(*se)->abort_tmo = -1;
	...
	_good(_sys_prop_get_str(ctx, sys_con_dir_path,
				      "persistent_address",
				      (*se)->persistent_address,
				      sizeof((*se)->persistent_address)/
				      sizeof(char)), rc, out);

	_good(_sys_prop_get_i32(ctx, sys_con_dir_path,
				      "persistent_port",
				      &((*se)->persistent_port)), rc, out);
	...
	_good(_sys_prop_get_i32(ctx, sys_con_dir_path, "port",
				      &((*se)->port)), rc, out);
